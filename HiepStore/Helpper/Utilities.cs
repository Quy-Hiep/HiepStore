...
namespace HiepStore.Helpper
{
    public static class Utilities
    {
        ...
        public static string GetRandomKeyNonSpecialCharacters(int length = 5)
        {
            //chuỗi mẫu (pattern)
            string pattern = @"0123456789zxcvbnmasdfghjklqwertyuiop";
            Random rd = new Random();
        ...
        public static async Task<string> UploadFile(Microsoft.AspNetCore.Http.IFormFile file, string sDirectory, string newname = null)
        {
            try
            {
                ...
                string path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "assets", "images", sDirectory);
                CreateIfMissing(path);
                string pathFile = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "assets", "images", sDirectory, newname);
